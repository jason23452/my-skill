...
這個 skill 用於既有專案 redesign。目標不是重寫整個產品，而是在現有技術棧與功能限制下，找出最能提升品質的設計問題並修正。若任務是整體系統排版或 `design/ui-layout.md`，本 skill 也負責先盤點現有 app shell、routes、shared components、page templates 與 responsive patterns，避免新的 layout spec 只服務單一 feature。
...
2. 診斷：依下方 audit checklist 找出沒有產品證據的 generic pattern、弱點、缺少的 state、RWD 與 accessibility 問題。
...
若任務是 system layout planning，掃描完成條件必須額外包含：

- route inventory：目前有哪些 routes / pages / feature folders，未來 PRD 可能新增哪些 surface。
- layout component inventory：是否已有 app shell、sidebar、top nav、drawer、sheet、tabs、table、form、card、empty/error/loading components。
- template inventory：dashboard、list/index、detail、form/editor、settings、state pages 是否已有 pattern。
- responsive inventory：現有 breakpoint、container、sidebar collapse、panel/drawer、table mobile fallback。
- gap summary：哪些 layout 規則缺失，會讓後續 feature 各自發明排版。

完成後輸出或文件必須把「既有可沿用 layout pattern」與「需要新增的 system layout policy」分開，不可直接跳到單頁 redesign。
...
- dashboard / admin 不必然錯，left sidebar 也不必然錯。先判斷現有 IA、route depth、使用頻率與主要任務是否支持；若支持，優化 sidebar 與 overview 的產品專屬資訊流；若不支持，再探索 top nav、floating command menu、collapsible panel、inspector workspace 或 document workspace。
...
- generic card：border + shadow + white background。若 card 是產品資訊物件的自然容器，可以保留並強化角色；若只是預設 scaffold，移除多餘 border 或 shadow，只保留真正傳達 hierarchy 的 surface。
