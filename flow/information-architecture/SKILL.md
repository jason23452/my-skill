...
- When a subagent provides `prd.md`, `spec.md`, `ux-spec.md`, `project-context.md`, or repo evidence, derive IA from those inputs first. Do not interview the user directly from a subagent with `question: deny`.
- If a structural decision is truly blocking, return it to the caller as a blocking `questionPlan`; do not invent alternate IA options and do not ask inline.
- When used inside `ux-spec-writer` or `wireframe-spec-writer`, do not write `INFORMATION_ARCHITECTURE.md`. Contribute the structural skeleton to the target artifact sections instead.
- Unless the user explicitly requests alternatives, produce one canonical IA recommendation. Do not create A/B/C navigation or layout options.

## Process

1. Look for an existing design brief at `.design/*/DESIGN_BRIEF.md` only when the task is design-brief based. If the caller already provided PRD/spec/UX artifacts, treat them as the design brief and do not search unrelated `.design` folders.
...
3. Resolve structural decisions from the provided artifacts and repo evidence. If information is missing, prepare recommended blocking questions for the caller instead of asking directly.
...
4. Produce the IA content in the caller's requested artifact. Only save `INFORMATION_ARCHITECTURE.md` when the user explicitly asks for a standalone IA document.
