...
```opencode-bootstrap-json
{
  ...
  "scaffoldCommand": [
    ...
    "node -e \"const fs=require('fs'),cp=require('child_process'),at=String.fromCharCode(64),bs=String.fromCharCode(92);const env={...process.env,PNPM_CONFIG_IGNORE_SCRIPTS:'true',CI:'1'};const clean=v=>{let s=String(v||'').trim().toLowerCase();for(const p of [at+'coss/','coss/',at+'coss:','coss:'])if(s.startsWith(p))s=s.slice(p.length);if(s.startsWith(at))s=s.slice(1);return s.split('').filter(ch=>'abcdefghijklmnopqrstuvwxyz0123456789-'.includes(ch)).join('')};const uniq=a=>Array.from(new Set(a));const requested=String(process.env.COSS_COMPONENTS||'').split(',').flatMap(v=>v.split(' ')).map(clean).filter(Boolean);const mode=String(process.env.COSS_BOOTSTRAP_MODE||'fast').trim().toLowerCase();const specs=mode==='full-style'||mode==='style'||mode==='full'?[at+'coss/style']:requested.length?uniq(requested.map(n=>at+'coss/'+n).concat(at+'coss/colors-neutral')):[at+'coss/ui',at+'coss/colors-neutral'];const timeout=Number(process.env.COSS_SHADCN_TIMEOUT_MS||600000),idle=Number(process.env.COSS_SHADCN_IDLE_MS||10000),stable=Number(process.env.COSS_SHADCN_STABLE_MS||15000),poll=Number(process.env.COSS_SHADCN_POLL_MS||2000),baseUi=at+'base-ui'+'/react';const sl=v=>{let s=String(v||'').split(bs).join('/');while(s.endsWith('/'))s=s.slice(0,-1);return s};const rd=f=>{try{return JSON.parse(fs.readFileSync(f,'utf8'))}catch{return {aliases:{}}}};const a2p=a=>{a=sl(a);return a.startsWith(at+'/')?'src/'+a.slice(2):a};const hasDep=n=>{const pkg=rd('package.json');return Boolean((pkg.dependencies||{})[n]||(pkg.devDependencies||{})[n])};const artifactsOk=()=>{const ui=a2p((rd('components.json').aliases||{}).ui||'src/components/ui');if(!fs.existsSync(ui))return false;const files=fs.readdirSync(ui).filter(n=>/\\.tsx?$/.test(n)&&n!=='index.ts');const min=requested.length?1:10;return files.length>=min&&hasDep(baseUi)&&hasDep('clsx')&&hasDep('tailwind-merge')};if(artifactsOk()){console.warn('coss shadcn registry artifacts already present; skipping install');process.exit(0)}const args=['dlx','shadcn@latest','add',...specs,'--yes','--overwrite'];const child=cp.spawn('pnpm',args,{stdio:['ignore','pipe','pipe'],env,shell:process.platform==='win32'});let last=Date.now(),done=false,early=false,forceTimer=null,firstOk=0;const armForce=()=>{forceTimer=setTimeout(()=>{if(!done)child.kill('SIGKILL')},5000);forceTimer.unref&&forceTimer.unref()};const pipe=(s,o)=>s&&s.on('data',b=>{last=Date.now();o.write(b)});pipe(child.stdout,process.stdout);pipe(child.stderr,process.stderr);const finish=(code,signal)=>{if(done)return;done=true;clearInterval(ticker);clearTimeout(timer);if(forceTimer)clearTimeout(forceTimer);if(early||code===0){process.exit(0)}if(artifactsOk()){console.warn('coss shadcn registry install exited after generating artifacts; continuing bootstrap');process.exit(0)}process.exit(code||1)};const stopEarly=msg=>{if(done||early)return;early=true;console.warn(msg);child.kill('SIGTERM');armForce()};const ticker=setInterval(()=>{const ok=artifactsOk(),now=Date.now();if(ok&&!firstOk)firstOk=now;if(!ok)firstOk=0;if(ok&&(now-last>=idle||now-firstOk>=stable))stopEarly('coss shadcn registry install generated stable artifacts; continuing bootstrap')},poll);ticker.unref&&ticker.unref();const timer=setTimeout(()=>{if(artifactsOk())stopEarly('coss shadcn registry install timed out after generating artifacts; continuing bootstrap');else{console.error('coss shadcn registry install timeout after '+timeout+'ms');child.kill('SIGTERM');armForce()}},timeout);timer.unref&&timer.unref();child.on('close',finish);child.on('error',e=>{console.error(e&&e.message?e.message:e);process.exit(1)})\"",
    "node -e \"const fs=require('fs'),cp=require('child_process');const pkg=JSON.parse(fs.readFileSync('package.json','utf8'));if(!((pkg.dependencies||{})['class-variance-authority']||(pkg.devDependencies||{})['class-variance-authority']))cp.execSync('pnpm add class-variance-authority',{stdio:'inherit',env:{...process.env,PNPM_CONFIG_IGNORE_SCRIPTS:'true'}})\"",
...
8. Greenfield bootstrap 會先依 placement resolver 建立或修正 `components.json`，再執行官方 CLI，讓 shadcn 依 resolved aliases 寫檔並安裝需要的依賴
9. Vite greenfield 預設使用較快的 existing-project 路徑：`add @coss/ui @coss/colors-neutral --yes --overwrite`，安裝所有 UI primitives 與 neutral tokens，但避免完整 style preset 的 font/layout 依賴
10. 若 `COSS_COMPONENTS` 有值，使用單次 batch install 指定元件並補 `@coss/colors-neutral`，例如 `COSS_COMPONENTS=button,spinner,card`
11. 只有使用者明確要求完整 theme、sidebar variables、fonts，或環境指定 `COSS_BOOTSTRAP_MODE=full-style` 時，才使用 `@coss/style`
12. 既有專案若已有 `components.json`，必須保留其 aliases 並使用 `add`；不可再次無條件執行 `init`
13. 若 shadcn 產生與 resolver 不一致的實體資料夾，bootstrap 必須把 UI 檔案收斂到 resolved UI directory
14. `components.json` aliases 必須指向 resolved components/ui/utils/hooks，不可一律覆寫成 shared
15. Machine-readable `opencode-bootstrap-json` command 中不要裸露 `@coss/<name>`；用 runtime 組字串避免 OpenCode reference resolver 把 registry spec 當本機路徑讀取
16. Machine-readable `opencode-bootstrap-json` 中，coss 只負責 scaffold/install，不宣告 `pnpm build`；Greenfield repo bootstrap 會等所有 selected skills 的 scaffold commands 完成後，再由基礎 frontend skill 的 verification 統一執行一次 build
17. 首次 `pnpm dlx shadcn@latest` 可能只是在下載 CLI package，超過 OpenCode shell 預設 120 秒；執行這條 bootstrap command 時外層 tool timeout 必須大於 `COSS_SHADCN_TIMEOUT_MS` 或至少 10 分鐘，否則會被外層 shell 提前殺掉，內部 watchdog 沒機會判斷 artifacts
...
官方 `get-started` 說明：新專案可用 `init @coss/style` 一次裝完整 preset；既有專案可用 `add @coss/ui` 加入所有 primitives；完整 theme、colors、sidebar variables 與 fonts 使用 `add @coss/style`；只需要 UI primitives 與 neutral colors 時使用 `add @coss/ui @coss/colors-neutral`。

Vite Greenfield bootstrap 的 machine-readable metadata 預設採用 faster path：
...
```bash
COSS_COMPONENTS=button,spinner,card
COSS_BOOTSTRAP_MODE=full-style
COSS_SHADCN_TIMEOUT_MS=600000
COSS_SHADCN_IDLE_MS=10000
COSS_SHADCN_STABLE_MS=15000
```

`COSS_COMPONENTS` 會讓 bootstrap batch install 指定 components 加上 neutral colors；`COSS_BOOTSTRAP_MODE=full-style` 才會改用 `@coss/style`。`COSS_SHADCN_TIMEOUT_MS` 是 Node wrapper 內部 timeout，不會延長 OpenCode / shell tool 的外層 timeout；若直接用 shell 跑，必須把 shell tool timeout 設到更大。
...
`@coss/style` 是完整 preset，會安裝所有 UI components、neutral color system、sidebar variables、base styles，並安裝 Inter / Geist Mono fonts。這是完整主題 opt-in，不是 Vite greenfield 的預設快速路徑。
...
3. 用官方 shadcn CLI 加入指定元件，或在 bootstrap metadata 透過 `COSS_COMPONENTS` batch install
