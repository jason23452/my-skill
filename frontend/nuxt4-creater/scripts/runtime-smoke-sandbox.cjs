...
function parseArgs(argv) {
  const parsed = {
    ...
    mode: process.env.OPENCODE_NUXT_RUNTIME_SMOKE_SANDBOX || "auto",
...
function installIfNeeded(cwd) {
  ...
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_INSTALL=${command}`)
...
function nuxtDevCommand(packageManager, host, port) {
  const args = ["nuxt", "dev", "--host", host, "--port", String(port), "--no-fork"]
...
function main() {
  ...
  const root = path.resolve(process.env.OPENCODE_NUXT_RUNTIME_SMOKE_ROOT || DEFAULT_ROOT)
  ...
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_SOURCE=${parsed.cwd}`)
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_SOURCE_FS=${fsType}`)
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_MODE=${useSandbox ? "sandbox" : "in-place"}`)
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_CWD=${runCwd}`)

  if (useSandbox) copySource(parsed.cwd, runCwd)
  const install = installIfNeeded(runCwd)
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_INSTALL_STATUS=${install.status}`)

  const packageManager = install.packageManager || detectPackageManager(runCwd) || "pnpm"
  const [program, args] = nuxtDevCommand(packageManager, parsed.host, parsed.port)
  console.log(`OPENCODE_NUXT_RUNTIME_SMOKE_DEV_COMMAND=${[program, ...args].join(" ")}`)
  ...
  child.on("error", (error) => {
    console.error(`OPENCODE_NUXT_RUNTIME_SMOKE_FAILED=${error?.message || String(error)}`)
    process.exit(1)
  })
