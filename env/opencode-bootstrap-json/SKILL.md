...
- Runner 可在 execution layer 將 heavy build verification mirror 到 container-native `/tmp/opencode-build` 執行；metadata 仍保留原始 build command 作為 canonical verification intent，source of truth 仍是 target repo。
- Runtime smoke 的語意是先啟動 dev server、等 readiness 成功，再執行 smoke test/probe；單次 health URL connection refused 只代表 server 尚未 ready，不是最終失敗。
