...
if (hasPackageJson) {
  ...
  lines = [
    ...
    "RUN python -m pip install --no-cache-dir uv && if [ -f pyproject.toml ]; then uv sync --no-dev; elif ls requirements*.txt >/dev/null 2>&1; then pip install --no-cache-dir -r requirements.txt; fi",
    "COPY . .",
